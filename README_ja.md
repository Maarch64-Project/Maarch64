...
**Maarch64** は、AArch64 (ARM 64-bit) バイナリを x86_64 ホスト環境上でネイティブ同等の速度で直接実行する、プラットフォーム非依存のユーザースペース動的バイナリ変換エンジン（Dynamic Binary Translator）です。

Linux の **ELF64** バイナリに加えて、macOS の **Mach-O ARM64 / Universal (Fat) Binary**、および **Android (APK / Native .so)** の自動判別ロード・実行に対応しています。
...
- **🌐 マルチOSバイナリ自動識別**: `AutoLoader` により、Linux ELF64 と macOS Mach-O 64-bit、Universal Fat Binary のバイナリ形式をマジックバイトから自動判定してロード。
- **🍏 逆 Rosetta 2 構想**: Intel Mac (macOS x86_64 ホスト) や Linux 上で Apple Silicon (ARM64) アプリを Darwin BSD システムコール、Mach トラップ、Objective-C ランタイム Thunk で動かす動作モデル。
- **🔬 ビット精度検証**: Unicorn Engine オラクルによる命令ステップごとのレジスタ差分検証 (`maarch64-diff-test`) を搭載。
- **🛡️ 100% メモリ安全な Rust 実装**: ゼロコピー mmap 仮想メモリ管理とスレッドセーフな JIT キャッシュ構造。
...
| **データ転送** | `MOV`, `MOVI`, `MOVK`, `MOVN`, `NOP` | コンテキストレジスタロード/ストア, ベクトル展開 |
...
| **🍏 macOS / Darwin** | `objc_msgSend`, `objc_getClass`, `sel_registerName`, `dispatch_async`, `MTLCreateSystemDefaultDevice` | ホスト macOS `libobjc.A.dylib` & `Metal.framework` |

---

## 📦 ワークスペース構造

Maarch64 は `Maarch64-Project` 組織配下のモジュール式 Cargo クレートで構築されています：
...
├── android/             # maarch64-android: Android APK パーサー & JVM エミュレーション
...
- 🌐 **[マルチOSバイナリ変換仕様 (docs/MULTI_OS.md)](docs/MULTI_OS.md)**: ELF64 vs Mach-O ARM64 ロード, Universal Fat Binary, System V vs Darwin ABI, Darwin BSD & Mach Traps。
...
### 1. バイナリの静的解析・インスペクション (`inspect`)

Mach-O、Universal Binary（Fat）、Linux ELF、Android APK などの内部構造（アーキテクチャ、セグメント、エントリポイント、要求される Thunk シンボル一覧など）を即座にインスペクションできます：

```bash
# Mach-O (macOS ARM64) バイナリの解析
...
# Cranelift JIT アクセラレーションを有効化して実行
target/release/maarch64 --jit tests/bin/lua -v

# 詳細なシステムコール/Thunk実行トレース付きで実行
target/release/maarch64 -v fixtures/macho_arm64_sample
```

### 3. macOS DMG ファイルの展開と実行（Linux 環境）

手元にある macOS 用 `.dmg` ファイルを Linux 上で展開し、含まれる ARM64 バイナリを解析・実行できます：

```bash
# 1. 7z コマンドで DMG ファイルを解凍
7z x <アプリ名>.dmg -o./extracted_app

# 2. .app/Contents/MacOS/ 配下の Mach-O バイナリを解析
target/release/maarch64 inspect extracted_app/*.app/Contents/MacOS/*

# 3. Maarch64 で実行（トレースモード）
...
- [x] **Phase 3: マルチOSアーキテクチャ & Mach-O / Darwin 対応** — `AutoLoader`, Universal (Fat) Binary スライス自動抽出, Mach-O 64-bit ARM64 ローダー, Darwin BSD syscall (30+ 種) & Mach Trap (`mach_task_self`, `mach_reply_port`, `mach_msg_trap`, `semaphore`), Objective-C / CoreFoundation Thunk。
